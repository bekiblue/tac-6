...
async def export_table(table_name: str):
    ...
    try:
        ...
        # Fetch all data from the table using safe query execution
...
async def export_results(request: ExportResultsRequest):
    ...
    try:
        ...
        for row in request.results:
            row_data = [row.get(col, '') for col in request.columns]
        ...
        # Return as streaming response with proper headers
        logger.info(f"[SUCCESS] Results exported: rows={len(request.results)}, columns={len(request.columns)}")
        return StreamingResponse(
            iter([csv_content]),
            media_type="text/csv",
            headers={
